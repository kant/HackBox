...
export const ensureHackathonCoAdmin = (hackathonId, userId, opts = { includeUser: false }) => {
    return client("hackathongs_coowners").where({ user_id: userId, hackathon_id: hackathonId }).then((rows) => {
        return rows[0];
    }).then((hackathonCoAdmin) => {
        if (!hackathonCoAdmin || !opts.includeUser) {
            return hackathonCoAdmin;
        }

        return ensureUser(userId).then((user) => {
            for (const key in user) {
                hackathonCoAdmin[key] = user[key];
            }
            return hackathonCoAdmin;
        })
    }).catch((err) => {
    ...
    if (hackathon_id) {
        ...
        */
        let rawQuery = [
            ...
            "as has_project,",
            "if(hco.user_id is null or hco.user_id = '', false, true) as is_admin",
            "from users",
            "inner join participants on participants.user_id = users.id",
            "left join hackathongs_coowners hco on hco.user_id = users.id and hco.hackathon_id = ?",
